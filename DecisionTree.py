...
def getAttributeCounts(examples, attrName, attrValues, className):
    ...
    for value in attrValues:
        pertinent = getPertinentExamples(examples, attrName, value)
        attributeCounts[value] = (getClassCounts(pertinent, className))
...
def setEntropy(classCounts):
    ...
    """
    H = 0
    classCountSum = float(sum(classCounts))
    for count in classCounts:
        count = float(count) / classCountSum
        H += -(count * log(count, 2))

    # print 'setEntropy returned: ', float(H)
    return float(H)
...
def remainder(examples, attrName, attrValues, className):
    ...
    """
    sumofClass = float(sum(getClassCounts(examples, className).values()))
    # print 'sum of class: ', sumofClass
    result = 0
    for value in attrValues:
        pertinentExamples = getPertinentExamples(examples, attrName, value)
        kclasscounts = getClassCounts(pertinentExamples, className).values()
        # print 'kclasscounts: ', kclasscounts
        result += float(sum(kclasscounts)) / sumofClass * float(setEntropy(kclasscounts))
    return result
...
def infoGain(examples, attrName, attrValues, className):
    ...
    """
    counts = getClassCounts(examples, className).values()
    # print setEntropy(counts), remainder(examples,attrName,attrValues,className)
...
def giniGain(examples, attrName, attrValues, className):
    ...
    gain = 0
    for value in attrValues:
        filtered = getPertinentExamples(examples, attrName, value)
        classCounts = getClassCounts(filtered, className).values()
        gain += float(len(filtered)) / len(examples) * giniIndex(classCounts)
...
def makeTree(examples, attrValues, className, setScoreFunc, gainFunc):
    ...
    return Tree(
        makeSubtrees(remainingAttributes, examples, attrValues, className,
...
def makeSubtrees(remainingAttributes, examples, attributeValues, className, defaultLabel, setScoreFunc, gainFunc):
    ...
    """
    # YOUR CODE HERE
    # IF examples is empty THEN RETURN leaf(default_val)
    if len(examples) == 0:
        # TODO is this valid?
        classValues = []
        classCounts = []
        for e in examples:
            if e[className] in classValues:
                classCounts[classValues.index(e[className])] += 1
            else:
                classValues.append(e[className])
                classCounts.append(1)
        modeindex = classCounts.index(max(classCounts))
        root = LeafNode(classValues[modeindex])
        return root
    # ELSE IF all examples have same label THEN RETURN leaf(label)
    allSame = True
    firstValue = examples[0][className]
    for e in examples[1:]:
        if e[className] != firstValue:
            allSame = False
    if allSame:
        root = LeafNode(firstValue)
        return root

    # best_q = question with highest info-gain
    best = remainingAttributes[0]
    for attr in remainingAttributes:
        if gainFunc(examples, attr, attributeValues[attr], className) > gainFunc(
                examples, best, attributeValues[best], className):
            best = attr

    newRemainingAttributes = list(remainingAttributes)
    newRemainingAttributes.remove(best)

    # node = new DT node with question best_q
    root = Node(best)
    for value in attributeValues[best]:
        pertinentExamples = getPertinentExamples(examples, best, value)
        # ELSE IF remaining questions is empty THEN RETURN leaf(majority-label(examples))

        if (len(pertinentExamples)) == 0:
            # Find the majority label
            subtree_default = getMostCommonClass(examples, className)
            child = LeafNode(subtree_default)
            root.children[value] = child
        # ELSE
        else:
            root.children[value] = makeSubtrees(
                newRemainingAttributes, pertinentExamples, attributeValues, className,
                defaultLabel, setScoreFunc, gainFunc)
